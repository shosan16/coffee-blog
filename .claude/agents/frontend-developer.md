...
1. `Read` ツールで `.workspace/design.md` を読み取る（存在しない場合はエラーを確認して次へ）
2. `Glob` ツールで対象ファイルを検索（例: `src/client/**/*.tsx`）
3. `Read` ツールで対象ファイルを読み取る
...
### ステップ 1: 設計確認（起動時アクションで完了）

設計ファイルの内容を確認し、実装方針を把握する。
