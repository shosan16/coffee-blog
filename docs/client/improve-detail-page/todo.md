...
- [x] ステップ番号デザインを変更（`h-12 w-12` → `h-8 w-8`）
- [x] ステップマーカーのスタイルを変更（`bg-accent text-accent-foreground text-xs font-bold`）
- [x] タイムラインの位置を調整（`left-6` → `left-4`）
- [x] 総抽出時間の表示位置を変更（CardHeader内から section-header と同じ行の右側に配置）

### 文言変更

- [x] 時間表示形式を変更（「30秒」→「0:00 - 0:20（20秒）」）
- [x] `useTimeFormat` フックに `formatTimeRangeWithDuration` 関数を追加（開始時刻、終了時刻、経過時間を計算）

### データ構造変更

- [x] 累積時間計算ロジックを実装（各ステップの `timeSeconds` から開始・終了時刻を算出）
- [x] ステップごとに `startSeconds`、`endSeconds`、`durationSeconds` を計算
- [x] 前ステップまでの累積時間を計算してstateに保持

### スタイリング変更

- [x] ステップマーカーのサイズを `h-12 w-12` → `h-8 w-8` に縮小
- [x] ステップコンテンツの余白を `p-4` → `p-3.5 md:p-4` に調整
- [x] 完了ステップのデザインは維持（変更なし）
