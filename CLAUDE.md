...
**重要**: エージェントは独立したプロセスとして動作するため、ファイル変更はファイルシステム上に実際に反映される必要がある。

#### エージェント側のルール

1. **実際にツールを使用する**: ファイル変更は必ず `Write` / `Edit` ツールを実行すること。変更内容の「説明」や「計画」だけを返してはならない
2. **変更の確認**: ファイル変更後、`Read` ツールで変更が正しく反映されたことを確認すること
3. **完了報告**: 作業完了時は、変更したファイルのパス一覧を明示的に報告すること

#### 親セッション側のルール

1. **変更の検証**: エージェント完了後、`git status` で変更されたファイルを確認すること
2. **未完了の検出**: `git status` で変更がない場合、エージェントがファイル変更を実行できなかった可能性がある。その場合は親セッションで直接実装するか、エージェントを再実行すること
3. **部分的な変更**: 一部のファイルのみ変更されている場合、残りは親セッションで完了させること
